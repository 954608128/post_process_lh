...
@jit(nopython=True)
def crop_pcdata(framdata, img, t_det):
    im_size=img.shape[0]
    img_corner = im_size // 2
    dist2pixel = im_size // 200
    ...
    for i in range(len(in_points)):
        in_points[i][0] += t_det[0]
        in_points[i][1] -= t_det[1]
...
def read_pcap_offline(filename):
    ...
    with open(filename,'rb') as f:
        ...
        for timestamp, raw_buf in pcap:  # timestamp时间戳，raw_buf包中的原始数据
            ...
            if list(packet.dst) == [192, 168, 3, 181] and list(packet.src) == [192, 168, 3, 80]:
...
@jit(nopython=True)
def filter_label_forNuScenes(labels,scores,num_points,mode):
    taked_indics=np.ones_like(labels)
    count_idx=0
    num_labels = len(labels)
    ...
    if mode==0:
        ##########针对不同种类使用不同的概率阈值选取合适的box_lidar#############
        for idx in range(num_labels):
            if labels[idx] == 0 and scores[idx] > 0.25 and num_points[idx]>0:
                taked_indics[count_idx]=idx
                count_idx += 1
            elif labels[idx] == 1 and scores[idx] >= 0.2 and num_points[idx]>0:
            # elif labels[idx] == 1 and scores[idx] >= 0.02:
                taked_indics[count_idx] = idx
                count_idx += 1
            elif labels[idx] == 2 and scores[idx] >= 0.02 and num_points[idx]>10:
                taked_indics[count_idx] = idx
                count_idx += 1
            elif labels[idx] == 3 and scores[idx] >= 0.02:
                taked_indics[count_idx] = idx
                count_idx += 1
            elif labels[idx] == 4 and scores[idx] >= 0.1:
                taked_indics[count_idx] = idx
                count_idx += 1
            elif labels[idx] == 5 and scores[idx] >= 0.02:
                taked_indics[count_idx] = idx
                count_idx += 1
            elif labels[idx] == 6 and scores[idx] >= 0.02:
                taked_indics[count_idx] = idx
                count_idx += 1
        ############################end##################
    elif mode==1:
